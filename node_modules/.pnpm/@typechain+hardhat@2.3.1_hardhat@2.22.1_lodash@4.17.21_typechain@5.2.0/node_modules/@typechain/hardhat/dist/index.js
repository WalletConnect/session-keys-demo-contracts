...
config_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_COMPILE_JOBS, 'Compiles the entire project, building all artifacts').setAction(async (taskArgs, { run }, runSuper) => {
    const compileSolOutput = await runSuper(taskArgs);
    await run(constants_1.TASK_TYPECHAIN_GENERATE_TYPES, { compileSolOutput });
    return compileSolOutput;
});
config_1.subtask(constants_1.TASK_TYPECHAIN_GENERATE_TYPES)
    ...
    const artifactPaths = lodash_1.uniq(artifactFQNs.map((fqn) => artifacts._getArtifactPathFromFullyQualifiedName(fqn)));
    ...
    const allFiles = typechain_1.glob(cwd, [`${config.paths.artifacts}/!(build-info)/**/+([a-zA-Z0-9_]).json`]);
    if (typechainCfg.externalArtifacts) {
        allFiles.push(...typechain_1.glob(cwd, typechainCfg.externalArtifacts, false));
    }
    const result = await typechain_1.runTypeChain({
        cwd,
        filesToProcess: needsFullRebuild ? allFiles : typechain_1.glob(cwd, artifactPaths),
        allFiles,
    ...
    if (!needsFullRebuild && typechainCfg.externalArtifacts) {
        const result = await typechain_1.runTypeChain({
            cwd,
            filesToProcess: typechain_1.glob(cwd, typechainCfg.externalArtifacts, false),
            allFiles,
            outDir: typechainCfg.outDir,
            target: typechainCfg.target,
            flags: {
...
config_1.task(constants_1.TASK_TYPECHAIN, 'Generate Typechain typings for compiled contracts').setAction(async (_, { run }) => {
    ...
    const allFQNNamesNested = compileSolOutput.artifactsEmittedPerJob.map((a) => {
        return a.artifactsEmittedPerFile.map((artifactPerFile) => {
            return artifactPerFile.artifactsEmitted.map((artifactName) => {
                return contract_names_1.getFullyQualifiedName(artifactPerFile.file.sourceName, artifactName);
            });
        });
    });
    return lodash_1.default(allFQNNamesNested).flatten().flatten().value();
}

...
<head>
    <title>Code coverage report for solidity-coverage/plugins/resources/nomiclabs.utils.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">solidity-coverage/plugins/resources</a> nomiclabs.utils.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">95.29% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>81/85</span>
            </div>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const shell = require('shelljs');
 ...
 */
function normalizeConfig(config, args=<span class="branch-0 cbranch-no" title="branch not covered" >{})</span>{
  let sources;
&nbsp;
  (args.sources)
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!path.isAbsolute(sources)) {
<span class="cstat-no" title="statement not covered" >    sources = path.join(config.paths.root, sources);</span>
  }
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (config.solidity &amp;&amp; config.solidity.compilers.length) {
...
&nbsp;
  ...
  config.logger = config.logger ? config.logger : <span class="branch-1 cbranch-no" title="branch not covered" >{log: null};</span>
...
&nbsp;
  try {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (semver.gt(hardhatPackage.version, '2.0.3')){
...
&nbsp;
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (solidity.overrides) {
    for (key of Object.keys(solidity.overrides)){
      <span class="missing-if-branch" title="else path not taken" >E</span>if (solidity.overrides[key].settings &amp;&amp; solidity.overrides[key].settings.viaIR) {
        return true;
      }
    }
  }
...
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (newCreateProviderSignature) {
...
<span class="cstat-no" title="statement not covered" >    provider = createProvider(</span>
      HARDHAT_NETWORK_NAME,
      networkConfig,
      env.config.paths,
      env.artifacts,
...
function collectTestMatrixData(args, env, api){
  if (args.matrix){
    mochaConfig = env.config.mocha || <span class="branch-1 cbranch-no" title="branch not covered" >{};</span>
 ...
 */
function <span class="fstat-no" title="function not covered" >tempCacheDir(</span>config){
<span class="cstat-no" title="statement not covered" >  return path.join(config.paths.root, '.coverage_cache');</span>
}
&nbsp;
/**
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (shouldKill) <span class="cstat-no" title="statement not covered" >process.exit(1)</span>
}
&nbsp;
module.exports = {
  normalizeConfig,

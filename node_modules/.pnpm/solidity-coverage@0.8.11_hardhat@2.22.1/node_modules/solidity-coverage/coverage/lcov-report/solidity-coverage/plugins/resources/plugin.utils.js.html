...
<head>
    <title>Code coverage report for solidity-coverage/plugins/resources/plugin.utils.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">solidity-coverage/plugins/resources</a> plugin.utils.js</h1>
        <div class='clearfix'>
 ...
 */
function reportSkipped(config, skipped=<span class="branch-0 cbranch-no" title="branch not covered" >[])</span>{
  let started = false;
  const ui = new PluginUI(config.logger.log);
&nbsp;
  for (let item of skipped){
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!started) {
 ...
 */
function <span class="fstat-no" title="function not covered" >setupTempFolders(</span>config, tempContractsDir, tempArtifactsDir){
<span class="cstat-no" title="statement not covered" >  checkContext(config, tempContractsDir, tempArtifactsDir);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  shell.mkdir(tempContractsDir);</span>
<span class="cstat-no" title="statement not covered" >  shell.mkdir(tempArtifactsDir);</span>
}
&nbsp;
/**
 ...
 */
function <span class="fstat-no" title="function not covered" >save(</span>targets, originalDir, tempDir){
  let _path;
<span class="cstat-no" title="statement not covered" >  for (target of targets) {</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    _path = path.normalize(target.canonicalPath)</span>
                .replace(originalDir, tempDir);
&nbsp;
<span class="cstat-no" title="statement not covered" >    fs.outputFileSync(_path, target.source);</span>
  }
}
&nbsp;
/**
 ...
 */
function <span class="fstat-no" title="function not covered" >checkContext(</span>config, tempContractsDir, tempArtifactsDir){
  const ui = <span class="cstat-no" title="statement not covered" >new PluginUI(config.logger.log);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (!shell.test('-e', config.contractsDir)){</span>
&nbsp;
    const msg = <span class="cstat-no" title="statement not covered" >ui.generate('sources-fail', [config.contractsDir])</span>
<span class="cstat-no" title="statement not covered" >    throw new Error(msg);</span>
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (shell.test('-e', tempContractsDir)){</span>
<span class="cstat-no" title="statement not covered" >    shell.rm('-Rf', tempContractsDir);</span>
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (shell.test('-e', tempArtifactsDir)){</span>
<span class="cstat-no" title="statement not covered" >    shell.rm('-Rf', tempArtifactsDir);</span>
  }
}
&nbsp;
&nbsp;
...
// =============================
&nbsp;
function assembleFiles(config, skipFiles=<span class="branch-0 cbranch-no" title="branch not covered" >[])</span>{
  let targets;
  let targetsPath;
&nbsp;
...
&nbsp;
function assembleTargets(config, targets=<span class="branch-0 cbranch-no" title="branch not covered" >[],</span> skipFiles=<span class="branch-0 cbranch-no" title="branch not covered" >[])</span>{
  const skipped = [];
  const filtered = [];
 ...
 */
function assembleSkipped(config, targets, skipFiles=<span class="branch-0 cbranch-no" title="branch not covered" >[])</span>{
  // Make paths absolute
...
&nbsp;
function loadSolcoverJS(config=<span class="branch-0 cbranch-no" title="branch not covered" >{})</span>{
  let solcoverjs;
  let coverageConfig;
  let log = config.logger ? config.logger.log : <span class="branch-1 cbranch-no" title="branch not covered" >console.log;</span>
...
&nbsp;
  ...
  config.mocha = config.mocha || <span class="branch-1 cbranch-no" title="branch not covered" >{};</span>
&nbsp;
 ...
 * @return {Promise}
 */
async function <span class="fstat-no" title="function not covered" >finish(</span>config, api){
  const {
    tempContractsDir,
    tempArtifactsDir
  } = <span class="cstat-no" title="statement not covered" >getTempLocations(config);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  shell.config.silent = true;</span>
<span class="cstat-no" title="statement not covered" >  shell.rm('-Rf', tempContractsDir);</span>
<span class="cstat-no" title="statement not covered" >  shell.rm('-Rf', tempArtifactsDir);</span>
<span class="cstat-no" title="statement not covered" >  shell.config.silent = false;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (api) <span class="cstat-no" title="statement not covered" >await api.finish();</span></span>
}
&nbsp;
module.exports = {
  assembleFiles,
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Thu Feb 29 2024 14:57:09 GMT-0800 (Pacific Standard Time)
            </div>
        </div>
